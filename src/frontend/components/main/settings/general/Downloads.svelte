...
const tabs = [
	{ id: 'track', icon: 'fa-music' },
	{ id: 'album', icon: 'fa-compact-disc' },
	{ id: 'playlist', icon: 'fa-list-ul' }
];
...
onMount(() => {
	opt = getDownloadOptions(activeTab);
});
...
async function updateDownloadOptions() {
	...
	await Promise.all([
		api.setPref(`dir.${activeTab}`, opt),
		sleep(300)
	]);

	updatingOptions = false;

	report.success(`${activeTab.capitalizeFirstLetter()} download options updated`);
}
...
<section class="relative panel">
	...
	<div class="flex items-center px-6 my-2">
		{#each tabs as tab}
			<button
				class="px-6 py-2 flex items-center gap-3 capitalize border-b-2 transition {activeTab === tab.id ? 'border-pulse-accent text-pulse-accent font-semibold tracking-wider' : 'border-transparent text-gray-400/60 hover:text-pulse-white/90'}"
	...
	<div class="mt-4 space-y-6 p-4">
		...
		<div>
			<label class="label">Overwrite action:</label>
			<RadioGroup options={overwriteOptions} bind:value={opt.overwrite} />
		</div>
