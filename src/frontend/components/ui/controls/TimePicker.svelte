...
import { clickOutside, portal, stopPropagation } from '../../actions';
	...
	{#if isOpen}
		...
		>
			...
			<div class="flex flex-col gap-1">
				...
				{#each minutes as min}
					<button 
						onclick={stopPropagation(() => setTime(h, min))}
				...
				<div class="mt-auto pt-2 border-t border-white/5">
					<button onclick={stopPropagation(() => isOpen = false)} 
						class="w-full text-[9px] font-bold text-gray-500 hover:text-white"
					>
						CLOSE
					</button>
				</div>
